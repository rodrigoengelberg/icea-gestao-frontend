...
class SearchComponent {
  ...
  constructor(_element: HTMLElement, options: SearchOptions) {
    ...
    this.suggestionsElement = document.querySelector(
      this._getSelector('suggestions')
    )
  ...
  private _focus = () => {
    ...
    if (this.mode === 'menu') {
      if (
        ...
        (this.resultsElement && !this.resultsElement.classList.contains(dNone))
      ) {
        this.menuObject?.show(this.element)
      }
    }
  ...
  private _keypress = () => {
    ...
    const minLength = parseInt(
      typeof option === 'boolean' ? (+option).toString() : option.toString()
    )
